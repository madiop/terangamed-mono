...
public class OpenApiConfig {
    ...
    public OpenAPI doctorServiceOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("TerangaMed — Doctor Service API")
                        .version("1.0.0")
                        .description("""
                                Microservice de gestion des médecins.

                                ## Authentification
                                1. Récupérer un JWT : `./scripts/get-token.sh admin`
                                   (le token est copié dans le clipboard).
                                2. Authorize ci-dessus → coller (Cmd-V) → Authorize.

                                Note : la création/modif de médecins exige le rôle ADMIN.
                                """)
